...
```{r}
Cdata <- read.csv("../data/creative1.csv") #Concatenated CRITT sg tables
...
```
Pauses and NKeys are very highly correlated (r = 0.93) so for further analysis we focus on pauses.
...
mean_keys_per_char_P_sqrt <- mean(sqrt((Cdata[ which(Cdata$Task == "P"),]$Nkeys)/Cdata[ which(Cdata$Task == "P"),]$LenS)) #P: 2.25397724051884 keys/character

mean_keys_per_char_T_sqrt <- mean(sqrt((Cdata[ which(Cdata$Task == "T"),]$Nkeys)/Cdata[ which(Cdata$Task == "T"),]$LenS)) #T: 3.71381669728488 keys/character

reduced_pc <- 100*(1-round(mean_keys_per_char_P/mean_keys_per_char_T, digits=3))

reduced_pc_sqrt <- 100*(1-round(mean_keys_per_char_P_sqrt/mean_keys_per_char_T_sqrt, digits=3))

print(glue("On average there are more keystrokes per character in translation ({mean_keys_per_char_T}, SD = {sd_keys_per_char_T}) than in post-editing ({mean_keys_per_char_P}, SD = {sd_keys_per_char_P}). This is a {reduced_pc}% reduction for the post-editing condition. Based on square-root-transformed values, this difference shrinks to {reduced_pc_sqrt}%.")) 
...
```
The median number of seconds per character spent on the task is slightly higher for translation, but the difference is much less pronounced for seconds than it is for pauses (above).
...
reduced_pc_log <- 100*(1-round(mean_seconds_per_char_T_log/mean_seconds_per_char_P_log, digits=3))

print(glue("On average there are more seconds per character in translation ({mean_seconds_per_char_T}, SD = {sd_seconds_per_char_T}) than in post-editing ({mean_seconds_per_char_P}, SD = {sd_seconds_per_char_P}). This is a {reduced_pc}% reduction for the post-editing condition. However, based on the logs, the relationship inverts with translation requiring {reduced_pc_log}% fewer seconds per character than post-editing.")) 


```

We plot seconds (`FDur_s`) as a function of source-segment length (in characters, `LenS`) and draw linear regression lines for post-editing (`P`) and translation (`T`) with 95% confidence intervals (shaded areas). 
```{r}
...
### Loading Data
```{r}
Cdata2 <- read.csv("../data/creative2a.csv") #Concatenated CRITT sg tables
...
```
The number of keystrokes per character is slightly higher when the texts are segmented at a sentence level.
...
reduced_pc_sqrt <- 100*(1-round(mean_Nkeys_per_char_para_sqrt/mean_Nkeys_per_char_sent_sqrt, digits=3))

print(glue("On average there are more keystrokes per character for sentence segmentation ({mean_Nkeys_per_char_sent}, SD = {sd_Nkeys_per_char_sent}) than for paragraph segmentation ({mean_Nkeys_per_char_para}, SD = {sd_Nkeys_per_char_para}). This is a {reduced_pc}% reduction for the paragraph condition. Based on suqare-root-transformed values, this difference shrinks to {reduced_pc_sqrt}% ")) 

```
The difference between sentence and paragraph segmentation is more pronounced in terms of keystrokes than in terms of trasnlating time. 
...
```
For six out of ten translators, there are more keystrokes per character when the texts are segmented at a sentence level
